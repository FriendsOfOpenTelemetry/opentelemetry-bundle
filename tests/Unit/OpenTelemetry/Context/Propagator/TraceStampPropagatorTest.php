...
class TraceStampPropagatorTest extends TestCase
{
    ...
    public function testSetIgnoresNonTraceparentKey(): void
    {
        $carrier = new Envelope(new \stdClass());

        $this->propagator->set($carrier, 'tracestate', 'vendor=value');
    ...
    public function testGetReturnsNullForNonTraceparentKey(): void
    {
        $traceParent = '00-0af7651916cd43dd8448eb211c80319c-b7ad6b7169203331-01';
        $carrier = new Envelope(new \stdClass(), [new TraceStamp($traceParent)]);

        $result = $this->propagator->get($carrier, 'tracestate');
    ...
    public function testKeysReturnsTraceparent(): void
    {
        $carrier = new Envelope(new \stdClass());

        $keys = $this->propagator->keys($carrier);

        self::assertSame([TraceContextPropagator::TRACEPARENT], $keys);
    }
